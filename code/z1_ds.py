...
logging.basicConfig(
    ...
    datefmt='%Y-%m-%d %H:%M:%S',
    handlers=[
        logging.FileHandler("/data/stance_detection_project/stance_detection_project/run_ds.log", encoding='utf-8')  # 运行日志自动保存到 run.log
    ]
)
logger = logging.getLogger(__name__)

# ===================== 核心配置（仅修改API_KEY） =====================
API_KEY = "sk-<API_KEY>"  # 填入你的 DeepSeek 官方 API Key
...
OUTPUT_JSON_PATH = "/data/stance_detection_project/stance_detection_project/results/test_predictions_real_z1_ds.json"

# 初始化客户端
client = OpenAI(api_key=API_KEY, base_url=BASE_URL)

# 模型参数
# 模型参数（适配 DeepSeek 官方API，去掉不支持的 top_p）
MODEL_PARAMS = {
    "temperature": 0.1,
    "max_tokens": 512
}
