...
class TargetRGCN(nn.Module):
    ...
    def __init__(self, num_node_features: int, hidden_dim: int,
        ...
        # 如果 torch_geometric 不可用，使用简化的 MLP 回退（保持接口兼容）
        if not _try_import_pyg():
            print("注意：torch_geometric 不可用，使用简化 MLP 代替 R-GCN")
            self.pyg_available = False
            self.mlp = nn.ModuleList()
            self.layer_norms = nn.ModuleList()

            # 第一层
            self.mlp.append(nn.Linear(num_node_features, hidden_dim))
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor,
        ...
        """
        if not getattr(self, 'pyg_available', False):
            # MLP 回退路径
...
class GraphEnhancedStanceDetector(nn.Module):
    ...
    def detect_with_llm(self, text: str, targets: List[Dict[str, Any]],
                        graph_context: str, llm_client, use_rgcn: bool = False) -> List[Dict[str, Any]]:
        """
        兼容外部调用的简化检测接口：直接使用图的文本描述和 LLM 客户端进行立场推理。
        Args:
            text: 原文
            targets: 目标列表（每项包含 id, normalized, granularity_level, type）
            graph_context: 由 RelationExtractor 生成的图文本描述（关系列表）
            llm_client: 具有 generate(system_prompt, user_prompt) 方法的客户端
            use_rgcn: 是否尝试使用 R-GCN（本接口不执行 R-GCN 编码）
        Returns:
            stances: list of {"target_id":..., "polarity":...}
        """
        # 构建目标列表文本
        target_lines = []
        for t in targets:
            target_lines.append(f"- {t.get('id')}: {t.get('normalized')} (层级:{t.get('granularity_level',2)}, {t.get('type','')})")

        target_list = "\n".join(target_lines)
        relation_list = graph_context or "无明显关系"

        rgcn_importance = "\n".join([f"- {t.get('id')}: 0.000" for t in targets])
        ...
        if not hasattr(llm_client, 'generate'):
            raise ValueError('llm_client must provide generate(system_prompt, user_prompt)')
        ...
        try:
            ...
            print(f"解析立场响应失败: {e}，原文片段: {response[:200]}")
