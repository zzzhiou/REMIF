...
v5 核心改进：
1. 整合 v4 权威版 prompt 到 Stage 1 目标抽取
2. 使用 DashScope API 直接调用 LLM（qwen-max）
3. 完整三阶段流程：
   - Stage 1: 目标抽取（v4 权威 prompt）
   - Stage 2: 关系抽取 + 异质图构建
   - Stage 3: 立场检测（LLM + 图结构信息）
4. 添加评估模块
"""
...
请抽取所有立场目标："""
    ...
    try:
        ...
        if match:
            ...
            return [
                {"id": f"T{i+1}", "text": t, "normalized": t, "type": "explicit", "granularity_level": 2}
...
def detect_stance(targets: List[Dict[str, Any]], relations: List[Dict[str, Any]],
                 graph: nx.MultiDiGraph, text: str) -> List[Dict[str, Any]]:
    """使用 LLM 进行立场检测（结合图结构信息）"""
    if not targets:
        return []

    # 构建目标列表
    target_list = "\n".join([
        f"- {t['id']}: {t['text']} ({t['type']})"
        for t in targets
    ])
...
def process_test_dataset(input_path: str, output_path: str, eval_output_path: str):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="处理样本"):
        ...
        # Stage 3: 立场检测（结合图结构）
        detected_stances = detect_stance(extracted_targets, relations, graph, text)
...
if __name__ == "__main__":
    ...
    output_path = 'results/test_predictions_real_v5.json'
    eval_output_path = 'results/test_evaluation_real_v5.xlsx'
