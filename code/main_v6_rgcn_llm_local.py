#!/usr/bin/env python3
"""
多目标立场检测完整 Pipeline - v6 本地模型版 (R-GCN + Local LLM)

v6 核心改进:
1. 使用本地 Qwen2.5-7B-Instruct 模型替代 DashScope API
2. 保持 v5 完整的三阶段 R-GCN + LLM 架构
3. 本地模型处理所有 LLM 任务：目标抽取、关系抽取、立场检测
4. 降低显存占用，支持 batch 推理

模型路径：/home/立场检测/model/Qwen2.5-7B-Instruct

使用方法:
    python main_v6_rgcn_llm_local.py --data_path PATH
"""
...
def load_local_llm_model(model_path: str = None):
    ...
    local_llm_model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map='balanced_low_0',  # 自动平衡多卡显存
        max_memory={0: "10GB", 1: "10GB", 2: "10GB", 3: "10GB"} if torch.cuda.is_available() else None,
        offload_folder="offload",  # 显存不足时使用磁盘
...
def call_llm_local(system_prompt: str, user_prompt: str, temperature: float = 0.1, max_tokens: int = 2048, model_path: str = None) -> str:
    """调用本地 Qwen2.5-7B-Instruct 模型"""
    global local_tokenizer, local_llm_model
    ...
    try:
        ...
        response = local_tokenizer.batch_decode(generated_ids, skip_special_tokens=True)[0]
        return response
...
def extract_targets(text: str) -> List[Dict[str, Any]]:
    """使用本地 LLM 进行目标抽取（v4 权威版）"""
    user_prompt = f"""文本：{text}

请抽取所有立场目标："""

    response = call_llm_local(TARGET_EXTRACTION_PROMPT, user_prompt, temperature=0.1, max_tokens=1024)
...
请判断关系（只返回 JSON，不要其他内容）："""
    ...
    response = call_llm_local(RELATION_EXTRACTION_PROMPT, user_prompt, temperature=0.1, max_tokens=512)

    if not response:
        return {"relation": "none", "direction": "none", "evidence": "", "confidence": 1.0, "reasoning": "本地模型调用失败"}
...
class RGCNLLMStanceDetector:
    ...
    def detect(self, graph: nx.MultiDiGraph, targets: List[Dict[str, Any]],
        ...
        # 5. 调用本地 LLM
        response = call_llm_local(STANCE_DETECTION_PROMPT, user_prompt, temperature=0.1, max_tokens=1024)
...
def process_test_dataset(input_path: str, output_path: str, eval_output_path: str, use_rgcn: bool = True, model_path: str = None):
    """处理测试集并评估（完整 R-GCN + 本地 LLM 版）"""
    print("="*80)
    print("测试集处理和评估（完整 R-GCN + 本地 LLM 版 v6）")
    print(f"本地模型路径：{model_path or LOCAL_MODEL_PATH}")
    print("="*80)

    # 测试本地模型
    print("\n加载本地 Qwen2.5-7B-Instruct 模型...")
    try:
        test_response = call_llm_local("你是一个助手。", "你好", temperature=0.1, max_tokens=50, model_path=model_path)
        if test_response:
            print(f"  本地模型加载成功：{test_response[:50]}...")
        else:
            print("  本地模型调用失败，请检查模型路径")
            return
    except Exception as e:
        print(f"  本地模型加载失败：{e}")
        print(f"  请确认模型存在于：{model_path or LOCAL_MODEL_PATH}")
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='多目标立场检测 Pipeline - v6 本地 LLM 版')
    parser.add_argument('--data_path', type=str, default='data/test-dataset-1k.xlsx',
                        help='数据文件路径')
    parser.add_argument('--output_path', type=str, default='results/test_predictions_real_v6.json',
                        help='输出 JSON 路径')
    parser.add_argument('--eval_output_path', type=str, default='results/test_evaluation_real_v6.xlsx',
                        help='评估输出路径')
    parser.add_argument('--no_rgcn', action='store_true',
                        help='不使用 R-GCN（纯 LLM 方案）')
    parser.add_argument('--model_path', type=str,
                        default='/home/songxuexian/立场检测/model/Qwen2.5-7B-Instruct',
                        help='本地模型路径')
