...
logging.basicConfig(
    ...
    datefmt='%Y-%m-%d %H:%M:%S',
    handlers=[
        logging.FileHandler("/data/stance_detection_project/stance_detection_project/runcot_ds_526.log", encoding='utf-8')  # 运行日志自动保存到 run.log
    ]
)
...
OUTPUT_JSON_PATH = "/data/stance_detection_project/stance_detection_project/results/test_predictions_real_zcot_ds_526.json"
...
实例： input"input": "今天菲律宾海警小船又在冲撞中国海警大船，越来越像当年的越南了。1979 年我已经 10 岁，对越南挑衅中国的做法记忆很深。现在的菲律宾在美国撑腰下，同样有些自我催眠，南海上不断挑衅、反复折腾，若不吃点教训，恐怕很难消停。"
      ...
      步骤 3:[最终结果]:
      严格按以下格式输出（每行一个，不要加括号，不要加任何多余说明）：
      目标: 菲律宾, 立场: 反对
      目标: 越南, 立场: 反对
      目标: 中国, 立场: 支持
      目标: 美国, 立场: 反对"
...
def parse_model_output(text: str) -> list:
    # 1. 剔除 <think> 标签及其内部的思考过程，防止干扰
    text = re.sub(r"<think>.*?</think>", "", text, flags=re.DOTALL)
    
    # 2. 终极正则：不再强制要求首尾有括号，只要有"目标:X, 立场:Y"就能抓取
    pattern = r"目标\s*[:：]\s*(.*?)\s*[,，]\s*立场\s*[:：]\s*(支持|反对|中立)"
